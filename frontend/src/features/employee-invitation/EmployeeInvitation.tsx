...
export const EmployeeInvitation = () => {
  ...
  const onSubmit = async (data: EmployeeInvitationFormData) => {
    try {
      ...
      navigate({ to: "/" }); // Navigate back to home or dashboard
    } catch (error) {
      toast.error("Failed to send invitation. Please try again.");
    }
  };

  const inputClass = "w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[#ff9500] bg-[#2c2c2c] text-white placeholder-gray-400";
